...
- **Skill** *(terme)* : un dossier contenant un `SKILL.md` (frontmatter `name` + `description` obligatoires) et d'éventuels **fichiers de référence** en lecture seule. Identifié par un **id stable** ; le **nom** est un libellé unique dans la banque, renommable sans casser ses référents. Créé par collage de texte dans la banque, ou importé.
...
- **Voyage par document** : l'export d'un pipeline embarque le contenu de ses skills dans un sidecar ; l'import ajoute à la banque ceux dont l'id est inconnu, en suffixant un nom déjà pris. Amende la frontière d'ADR-0059 : un skill est du contenu, pas une configuration d'instance.
