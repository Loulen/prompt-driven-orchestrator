...
//! - inherits the daemon environment plus an injected `MAESTRO_TARGET_REPO`;
...
pub const GUARD_TIMEOUT_MS_OVERRIDE_ENV: &str = "MAESTRO_GUARD_TIMEOUT_MS";
...
pub const TARGET_REPO_ENV: &str = "MAESTRO_TARGET_REPO";
...
/// daemon environment is inherited and `MAESTRO_TARGET_REPO` is injected.
...
mod tests {
    ...
    #[tokio::test]
    async fn injects_maestro_target_repo_env_var() {
        let repo = tempfile::tempdir().unwrap();
        let result = run_guard(
            "printf '%s' \"$MAESTRO_TARGET_REPO\"",
            repo.path(),
