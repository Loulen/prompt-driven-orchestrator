...
**Coût dérivé / coût rapporté** *(termes, ADR-0052)* : un coût **dérivé** est recalculé par PDO (tokens × table de prix résolue) — c'est le chemin de `claude`. Un coût **rapporté** est celui que le harnais **compte lui-même** dans son unité de facturation, que PDO convertit par une **constante publiée** et **sans passer par la table de prix** : les buckets de cache ne se mappent pas d'un harnais à l'autre, donc un recalcul uniforme fabriquerait un chiffre faux (ADR-0045). Les deux formes s'additionnent en dollars, mais un total de Run est **ventilé par harnais** : « X via `copilot`, Y via `claude` ». Un total **indisponible** (un nœud sur un harnais sans source de coût, #553) n'efface pas la ventilation : c'est la **somme** qui est refusée, pas la connaissance — les tranches calculables se disent sous le « — » et sa raison (ADR-0052 §3, FP #617). _Éviter_ : recalculer un coût rapporté depuis ses tokens (double-comptage du cache) ; convertir en euros (le chemin de lecture reste local, ADR-0034) ; sommer des unités natives entre harnais ; lire une tranche comme une fraction de total.
