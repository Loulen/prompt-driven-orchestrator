...
fn build_tmux_script_uses_exec_bash_and_invokes_claude() {
    ...
    assert!(
        script.contains("$(cat ") && script.contains("/tmp/pdo-test/solo-iter-1.md"),
        "script must `cat` the prompt file inside command substitution; got: {script}"
    );
    assert!(
        script.contains("export PDO_RUN_ID=") && script.contains("run-abc"),
        "script must export PDO_RUN_ID; got: {script}"
    );
    assert!(
        script.contains("export PDO_DAEMON_URL=") && script.contains("http://localhost:5172"),
        "script must export PDO_DAEMON_URL; got: {script}"
    );
}
...
async fn tmux_session_alive_after_run_spawn() {
    ...
    let session = format!("pdo-{run_id}-{NODE_ID}-iter-1");
