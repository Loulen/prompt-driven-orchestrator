...
- **First-party ≠ déclaré** : un **harnais first-party** a ses capacités implémentées en code et embarquées dans le binaire ; un **harnais déclaré** n'a qu'un descripteur sur disque, donc il se lance, s'attache, se reprend, et se complète à la main — sans coût ni fin de tour. Les deux sont légitimes. La différence est **publiée** dans le *tableau de support* de `docs/reference/harnesses.md`, **généré depuis le code** pour qu'il ne puisse pas mentir, avec la dernière version validée de chaque binaire. _Éviter_ : « harnais supporté » sans dire quelles capacités ; « non supporté » pour un harnais déclaré (il tourne).
- **Prérequis de harnais** *(terme)* : ce que PDO **suppose configuré** et ne configure pas — authentification, dossier de travail approuvé, version installée. PDO ne met en scène le home d'aucun harnais hors sandbox : c'est un prérequis documenté dans `docs/reference/harnesses.md`, pas du code. _Éviter_ : traiter un prérequis absent comme un bug de PDO ; « PDO installe le harnais » (il ne l'embarque ni ne le fournit).
...
### Routage — `mode` + `waypoints` (#154) et grille de câblage (ADR-0072, ADR-0076)
...
- **Grille de câblage** *(terme)* : le treillis sur lequel un tracé snappe. Son pas est une **taille de grille** (ADR-0076, supersede ADR-0072). La grille décorative du fond est une autre chose, dessinée au même pas. Son **feedback** pendant le geste (`none` / `dots` / `lines`, défaut `dots`) est un réglage de confort par navigateur. _Éviter_ : « snap grid utilisateur », « pas de grille du projet ».
- **Taille de grille** *(terme, `S` / `M` / `L` = 20 / 30 / 40 px, défaut `M`)* : le pas de la grille de câblage d'un pipeline. Le pipeline peut **porter la sienne** (`grid_size`, choisie dans le Pipeline Inspector, écrite dans le fichier : un pipeline partagé se rend alors pareil partout). Sinon il suit le **défaut global**, un réglage par navigateur (Settings › Interface). Changer de taille ne **re-snappe jamais** les waypoints existants. `grid_size` est du **layout**, comme `mode` + `waypoints`. _Éviter_ : « zoom de grille », « densité ».
...
**Préambule de base** *(terme, #588)* : le préambule est **identique pour tous les nœuds agentiques** et ne parle ni d'interactivité ni d'orchestration. Ces conduites arrivent par un **Skill de toggle** : activer `interactive` (resp. `orchestrator`) ajoute la référence `pdo-interactive` (resp. `pdo-orchestrate`) aux skills du nœud, retirable avec avertissement, et PDO dépose la ligne d'invocation `/pdo-interactive` (resp. `/pdo-orchestrate`) après le prompt du rôle au spawn. Le préambule dit à **tout** nœud de lancer `pdo complete` ; c'est le **refus** du daemon qui apprend à l'agent qu'il est interactif (ADR-0068), jamais une consigne préalable. _Éviter_ : « préambule interactif », « amendement Orchestration » comme bloc de prose (il ne reste que la ligne d'invocation).
...
- **Skills actifs** *(terme, #847 — remplace « skills effectifs », 2026-09-22)* : l'**union** des skills sélectionnés sur les quatre tiers Configuration d'instance → Projet → Run → Node. Additif strict : aucun tier ne retire un skill hérité. Le Node référence les skills par **id** dans le Document de pipeline (nom en libellé) ; c'est de la **sémantique** (diff sémantique, `content_hash`). Un Node `script` n'en porte pas ; les sessions d'infra reçoivent ceux du Run. Le renommage vaut pour le glossaire, l'UI (« active skills »), le code et l'API : aucune dette sous l'ancien nom. _Éviter_ : « skills effectifs » (ancien terme), « skills du run » pour l'union, « skills installés » (installer, c'est importer dans la banque).
- **Sélecteur de skills** *(terme, #847)* : le contrôle, identique aux quatre tiers, qui règle les skills d'un tier et lit les skills actifs. **Replié**, il tient sur une ligne compacte : icône, titre « Skills », sous-ligne discrète « n active skills » — le **compte seul**, jamais les noms (trop nombreux pour servir), et une **icône d'alerte** à droite quand un skill sélectionné n'existe plus dans la banque (le message est en tooltip). **Déplié**, l'arbre de la banque montre les skills cochés ; un skill hérité d'un tier supérieur est coché, grisé, non décochable, son tier d'origine écrit à droite ; un skill propre se retire en le décochant ; les skills disparus de la banque sont listés barrés en tête, décochables. En **lecture seule** (nœud en cours de run, détail de trigger), le sélecteur se déplie quand même, cases figées. _Éviter_ : « liste des skills effectifs » sous le bouton (supprimée : elle doublait le popover), « étiquettes »/« chips » de skills (écartées le 2026-09-22 : une valeur par chip n'apporte rien que l'arbre coché ne dise déjà).
- **Skill seedé** (#722, #588) : au démarrage du daemon, les skills `pdo-orchestrate` et `pdo-interactive` sont écrits dans la banque — créé si absent, son contenu réécrit quand la copie embarquée dans le binaire diffère (le frontmatter porte `skill_version`, versionnée avec PDO) — dans un dossier « PDO », et **verrouillé** : ni édition ni suppression via la surface banque (`SkillError::Locked`, badge « Managed by PDO » côté UI). Deux skills seedés, un par toggle de nœud (cf. *Préambule de base*) — le mécanisme reste minimal (ADR-0064). _Éviter_ : « skill du produit » (il vit dans la banque de l'instance comme tout skill, sélectionnable et livrable), « mise à jour de la banque » (le daemon ne réécrit que ce skill).
...
- **Un Node peut être marqué `interactive: true`** : sa complétion est **gardée** jusqu'à ce que l'utilisateur la force (« Mark complete ») ou la **libère** à l'agent (« Mark ready for completion », ADR-0068). Il n'est **pas** `awaiting_user` d'office : il le devient quand l'agent le **déclare** (cf. *Attente déclarée*, ADR-0069).
...
**Message de pilotage** *(terme, #790 — steering message)* : tour **humain** tapé dans la session vivante d'un NodeRun **après son lancement**. N'en sont pas : le **prompt de lancement** (premier tour utilisateur de la session principale, exclu par position, y compris après une reprise), et un **message runtime**. Compté par exécution réussie, dérivé à la lecture des transcripts résolus par le fold de coût, jamais persisté (ADR-0029) ; couverture harnais par harnais (ADR-0045), un harnais non instrumenté dit « — ». Lisible dans Stats › Performance (métrique **Steering**) par Pipeline → Node et par Modèle → effort → Pipeline → Node (ADR-0065), même périmètre que le pic de contexte et la durée (scripts exclus, sous-agents non comptés, Infrastructure en « — »). C'est la mesure d'**autonomie réelle** d'un rôle. _Éviter_ : « message utilisateur » (ambigu avec le `type:"user"` des transcripts, qui inclut les résultats d'outils), « intervention », « relance ».

**Message runtime** *(terme, #790)* : texte que le **daemon** colle lui-même dans la session d'un harnais (relance frontmatter ADR-0035, envoi au manager). Toujours **préfixé `[pdo-runtime]`**, posé au point unique d'écriture et jamais par les appelants ; c'est ce préfixe qui le distingue d'un message de pilotage dans un transcript, où les deux sont sinon indiscernables. _Éviter_ : « message système » (c'est le vocabulaire du harnais), un message runtime sans préfixe.

**Taux d'exécutions pilotées** *(terme, #790 — steered executions)* : part des exécutions réussies d'un Node (ou d'un couple modèle × effort) ayant reçu **au moins un** message de pilotage ; affiché avec sa couverture (« 23 % · 7/30 »). Ne porte que sur les exécutions au transcript lisible.

### Statistiques d'instance (cockpit, #377)

Surface **Stats** superposée en pleine fenêtre, avec navigation latérale et période commune aux sections : agrégats **transverses** filtrables par période (runs/erreurs/sessions, fires par pipeline, coûts), à distinguer des Statistiques de Run. Sessions et Cost ventilent les harnais dans des barres empilées par période et dans un tableau hiérarchique Pipeline → Nodes ; Cost affiche total et **médiane** par exécution, tandis que Sessions porte les nombres d'exécutions. Les noms visibles suffisent, l'interface n'expose pas leurs identifiants techniques. Une lecture secondaire reprend la même hiérarchie par Projet → Pipelines → Nodes, et une troisième **par Modèle → effort → Pipelines → Nodes** (Cost et Performance, ADR-0065), le drill d'un Node se terminant par ses couples modèle × effort ; un Run sans Projet nommé porte le nom de son dépôt primaire. Les sessions propres au Run forment la ligne **Infrastructure**, tandis que les subagents appartiennent au Node parent (ADR-0058) ; un coût impossible à rattacher reste dans **Non attribué**, jamais dans une catégorie inventée. Une exécution de Node est un démarrage, donc chaque tour de boucle et chaque redémarrage compte séparément dans la distribution. La période sélectionne les Runs démarrés dans sa fenêtre et inclut toutes leurs exécutions et leur coût complet. Une médiane de coût ne porte que sur les exécutions au coût lisible ; elle garde le préfixe `~`, et son détail donne la couverture. Le marqueur `†` reste réservé à une borne basse incomplète. Un harnais n'est absent du tableau que s'il n'a aucune exécution sur la période ; un coût inconnu reste visible comme « — », jamais comme `$0`. Les événements antérieurs au multi-harnais sont attribués à `claude`, seul harnais agentique alors disponible ; les Nodes `script` restent hors compte. Les couleurs sont stables par nom de harnais. Le calcul ne se rafraîchit pas en arrière-plan : l'utilisateur déclenche **Refresh** et voit l'heure du dernier calcul. Les tarifs vivent dans le panneau secondaire **Pricing details**, avec leur synchronisation, la table résolue et ses avertissements. Les agrégats sont dérivés à la lecture, **jamais matérialisés** (ADR-0029), et restent exprimés en USD.

**Médiane, jamais la moyenne** *(règle, #808)* : partout où Stats résume une distribution (tri, libellé, carte, box-plot, Cost par exécution), la valeur centrale est la **médiane** (R-7). La moyenne n'apparaît que dans le tooltip d'une distribution, comme sixième valeur. _Éviter_ : « avg », « moyenne » dans une carte ou un tri.

**Runs terminés seulement** *(terme, #808 — completed only)* : filtre de **cohorte** qui restreint une section de Stats aux Runs arrivés à `completed`. **Par onglet**, avec son propre défaut : activé sur Performance, désactivé sur Overview, Sessions, Triggers et Cost (Overview affiche alors zéro erreur, rien n'est masqué). Cohorte par défaut = tous les Runs démarrés dans la période, quel que soit leur statut final, où un Node réussi dans un Run `failed` compte. _Éviter_ : « runs réussis » (un Run `completed` peut contenir des nœuds échoués puis relancés), « filtre de statut », « bascule de la coquille » (il n'y a plus de filtre commun aux onglets hors la période).

**Mode de durée** *(terme — Total / Active / Waiting)* : lecture à trois positions de la durée d'une exécution de Node sur Performance, une distribution par mode, toujours calculées ensemble par le daemon d'un seul fold. **Total** : durée murale `NodeStarted → NodeCompleted`. **Active** (défaut) : Total moins l'**attente déclarée**. **Waiting** : l'attente déclarée seule, autrement dit le temps de cerveau humain qu'une exécution a consommé. L'attente déclarée mesurée = intervalles entre un `NodeAwaitingUser` de cause `declared` ou `completion_not_released` et le premier `NodeResumed`, `NodeCompletionReleased` ou événement terminal du même `(node, iter)`. La ligne Infrastructure porte la somme des attentes des Nodes du Run. Ne comptent **pas** comme attente : la cause `children_pending` (un orchestrateur sur `pdo run wait` travaille) et l'attente **dérivée** d'un parent dont un enfant attend (rien n'est écrit dans l'event log, ADR-0069 §4 : non mesurable, assumé) ; rien n'est jamais inféré d'un transcript ni d'un silence. Une **couverture** (« n exécutions sur N ont déclaré au moins une attente ») est disponible derrière un « i » survolable en Active et Waiting : pour les exécutions sans attente déclarée, Active = Total et Waiting = 0, ce qui est le cas de tout l'historique antérieur à l'attente déclarée. _Éviter_ : « exclude user wait » (l'ancienne bascule binaire), « temps de travail », « durée nette », « durée active » comme nom du mode.

**Genre de nœud** *(terme, #808 — node kind)* : lecture transverse d'un Node pour les filtres de Stats : **Interactive** (`interactive: true`), **Orchestrator** (`orchestrator: true`), **Standard** (ni l'un ni l'autre). Un Node peut être Interactive et Orchestrator à la fois et matche alors les deux. Les deux drapeaux sont **gelés dans l'événement de démarrage** du Node (ADR-0007), avec repli sur le snapshot du Pipeline pour l'historique antérieur. Le filtre de Performance est une multi-sélection, tout coché par défaut ; il porte sur les lignes Node des trois arbres, jamais sur Infrastructure ; les sous-agents suivent leur Node. À ne pas confondre avec le **type de node** (`agent` / `script` / `merge`). _Éviter_ : « type de nœud » pour ce sens, « catégorie », « rôle » (= le prompt).

**Niveau de zoom d'un box-plot** *(terme, #808)* : réglage à trois positions, commun à tous les box-plots de Performance : **Full** (moustaches min–max, défaut), **Fenced** (moustaches aux **bornes de Tukey** : plus petite et plus grande observation dans `[Q1 − 1,5 IQR ; Q3 + 1,5 IQR]`, calculées par le daemon, les outliers sortent du cadre), **Box** (boîte Q1–Q3 et médiane seules). Aucun point individuel : le daemon ne renvoie jamais les observations. **Axe partagé / échelles indépendantes** *(terme)* : avec **« independent scales »** (défaut) chaque ligne d'un tableau a son propre max d'axe X ; désactivé, l'axe est commun à toutes les lignes visibles et se cale sur le max du niveau de zoom choisi. _Éviter_ : « outliers masqués » (ils sont hors cadre, pas retirés de la distribution), « échelle log ».

**Réglages de Stats éphémères** *(règle)* : tout réglage de la surface Stats (période, cohorte par onglet, mode de durée, genres de nœud, zoom, échelles, axe, interrupteur Uncombined, tri) revient à son **défaut à chaque ouverture** de Stats ; s'en écarter est un geste manuel et volontaire, conservé seulement tant que Stats reste ouvert (changer d'onglet ne remet rien). Aucune pastille ne signale un écart aux défauts : la bande de filtres de l'onglet est la seule source de vérité. _Éviter_ : « préférence par navigateur » pour un réglage de Stats (réservé au thème), « filtres sales / dirty ».

**Absorption** *(terme, #888 — UI « Combine »)* : décision de l'opérateur, enregistrée dans l'instance, qu'une ligne de Stats (un Pipeline, un Node d'un Pipeline ou un Modèle) **absorbe** d'autres lignes du même niveau. L'**absorbant** garde son nom et cumule les exécutions, le coût et les fires de ses **absorbés** dans tous les onglets qui ont cette dimension. Il n'y a qu'un niveau : un absorbé n'absorbe rien, et absorber un absorbant lui prend ses membres. Sous un Pipeline absorbant, les Nodes de même id se réunissent d'eux-mêmes ; une absorption de Nodes ne traverse jamais deux Pipelines distincts, et retirer un Pipeline absorbé fait sortir ses Nodes de l'absorption de Nodes de son ancien absorbant. Une absorption de Modèles se pose sur l'axe « By model » de Cost et de Performance et s'applique partout où le modèle apparaît, couples modèle × effort des Nodes compris ; ses couples s'y réunissent par effort. Sur ce même axe, une **absorption d'efforts** réunit des efforts d'un seul modèle (typiquement *not set* dans un effort explicite) et s'applique elle aussi aux couples des Nodes. Dans le détail d'un Node, une **absorption de couples** réunit des couples modèle × effort quelconques en une ligne ; elle reste locale à cette ligne Node (arbres Pipeline et Projet) et ne remonte jamais sur l'axe « By model ». Le global s'applique avant le local ; un couple touché par une absorption globale le montre, grisé, sans pouvoir la défaire depuis le Node. Une absorption d'efforts ou de couples reste collée aux exécutions de la ligne où elle a été posée, quoi qu'il arrive ensuite à cette ligne (ADR-0078). Renommer un Pipeline crée l'absorption ancien → nouveau. L'absorption est propre à l'instance (elle ne voyage pas avec l'export d'un Pipeline), survit à la suppression d'un Pipeline et se défait membre par membre ; l'interrupteur éphémère **Uncombined** montre les lignes d'origine. Elle s'applique à la lecture, jamais dans l'event log (ADR-0077). _Éviter_ : « regroupement » (déjà l'axe de Stats et la définition du Projet), « fusion », « merge » (Merge node, merge-back), « alias » (réservé aux modèles épinglés), « normaliser l'effort » (rien n'est réuni sans geste).

**Pas de référence de décision dans l'interface** *(règle)* : un texte visible (libellé, tooltip, en-tête, carte) dit la règle en mots ; les numéros d'ADR et d'issue restent dans le code, les docs et les commits. _Éviter_ : « (ADR-0069) » et consorts dans une chaîne affichée.
...
**Ref de Run** *(terme)* : un point git persistant connu du Run, proposé comme source ou destination de la Review : le **point de fork**, le **tip du Run**, et pour chaque nœud livré ses SHA **before/after** de livraison, libellés par le nom du nœud et son `iter`. Un nœud en cours y ajoute sa branche de sous-worktree vivante. Tant que le worktree partagé du Run existe, le ref **`worktree`** (« Working tree ») pointe sur son arbre de travail — commits *et* modifications non commitées, fichiers non suivis inclus, `.pdo/` exclu — et son « SHA » est l'id d'un **arbre instantané** (snapshot, pas un commit) que les commentaires de review ancrent (#835). Les branches de sous-worktree ne survivent pas au merge-back, donc la Review ne raisonne jamais « par branche de nœud » (ADR-0067). Défaut : point de fork → `worktree` tant que le worktree existe (aucun commit requis pour voir le travail d'un Run en cours), point de fork → tip du Run ensuite ; les deux coïncident sur un arbre propre. _Éviter_ : « branche du nœud » comme identité stable ; « il faut commiter pour voir la diff ».
...
- **Attente d'enfants** *(terme, #588, ADR-0069)* : `pdo run wait [--all] [--timeout <s>]`, **pull bloquant** depuis la session de l'orchestrateur : rend la main quand un enfant de ce nœud (tous, avec `--all`) atteint un état **terminal** après le début de l'appel, et imprime son verdict ; un délai écoulé sort par un code dédié qui n'est pas un échec ; aucun enfant actif à l'appel ⇒ `noop` honnête (ADR-0025). Le daemon ne **pousse jamais** de message dans le pane du parent (ADR-0051/0068). Un enfant `awaiting_user` ne réveille pas l'appel : son attente est **propagée au nœud parent par dérivation à la lecture** (comme `stalled`, #180 : rien n'est écrit dans l'event log du parent, la levée est automatique, la profondeur de l'arbre est gratuite), avec la raison « child run <nom> is awaiting you » en bannière. Le skill `pdo-orchestrate` décrit cet usage ; « suivre ses enfants » ne se fait plus par sondage périodique. _Éviter_ : « notification », « push », « canal » (c'est un long-poll sur l'event log, l'enfant n'émet rien de particulier).
...
- **Branche source** *(terme, #571, #800)* : le point de coupe d'un Run — la réf de branche depuis laquelle sa branche `pdo/run-<id>` est créée. Une branche **locale** (`feature-x`) ou une **branche de suivi remote** (`origin/feature-x`), stockée **verbatim** — on ne réécrit jamais ce que l'utilisateur a choisi. Optionnelle : absente, le HEAD du repo cible fait défaut. La coupe elle-même ne fetch pas : une réf de suivi vaut ce que le dernier fetch en a fait (même contrat que la base des secondaires, ADR-0042). La **fraîcheur est visible et actionnable** au moment de choisir (formulaire : fetch à l'ouverture, écart amont affiché, geste de synchro explicite) et **assurée avant la coupe d'un Trigger** (fetch, jamais de fast-forward) — contrat → ADR-0070. _Éviter_ : « branche distante » au sens « réf vivante côté serveur », « pull automatique » (PDO ne merge ni ne rebase jamais une branche locale), « sync » comme nom d'une mesure (c'est le geste).
- **Écart amont** *(terme, #800)* : pour une branche locale qui a une branche de suivi, le couple `n↑ m↓` — `n` commits que la locale a en plus, `m` que la branche de suivi a en plus — **tel qu'au dernier fetch**. Une branche sans suivi ou une réf de suivi n'ont pas d'écart amont. Un fetch impossible (hors ligne, sans remote, clé absente) rend l'écart **inconnu**, daté du dernier fetch réussi ; il ne bloque jamais un lancement.
- **Synchro de la branche source** *(terme, #800)* : le geste explicite du formulaire de lancement : fetch de toutes les remotes (avec élagage des réfs disparues), puis **fast-forward** de la branche locale choisie si et seulement si c'est sans perte (arbre du checkout propre, pas de divergence, branche non checkoutée ailleurs) ; sinon la personne se voit proposer de **couper sur la branche de suivi** (`origin/<branche>`) à la place. PDO ne pousse jamais. _Éviter_ : « pull », « sync » au sens VS Code (pull + push).
- **Dérive de la source** *(terme, #800)* : pour un Run, le couple `n↑ m↓` entre sa branche et sa branche source depuis le point de fork — `n` commits faits par le Run, `m` commits arrivés sur la source (locale, et sa branche de suivi si elle en a une). Recalculée à chaque affichage sur les réfs locales, rafraîchie côté distant par le même geste de fetch ; jamais de fetch implicite à l'affichage. Non nulle côté `m`, elle annonce le merge ou rebase à venir au retour du Run.
...
**Terminal partagé** *(#867, ADR-0075)* : quand deux **postes** affichent le même terminal (nœud, Manager, Assistant de bibliothèque, Shell de run), un seul le **pilote** ; les autres sont **spectateurs**. Seul, ou avec deux onglets d'un même navigateur, il n'y a ni rôle ni friction.

- **Poste** *(terme)* : un navigateur, identifié durablement côté client — pas une personne (pas d'auth), pas une IP, pas un onglet. Deux onglets du même navigateur sont le même poste. _Éviter_ : « utilisateur », « client » (= client tmux), « origine » (= l'en-tête `Origin` du contrôle WS).
- **Pilote** *(terme)* : le poste dont la taille fixe celle du terminal et le seul dont les frappes passent. Le premier arrivé pilote ; quand il part, la main revient au seul poste restant, ou au premier arrivé parmi les restants. _Éviter_ : « contrôleur », « owner ».
- **Spectateur** *(terme)* : un poste qui voit le terminal en temps réel, en lecture seule, sans peser sur sa taille. _Éviter_ : « viewer » seul (le pilote regarde aussi), « observateur ».
- **Prise de main** *(terme)* : le geste explicite (icône main au-dessus du terminal) par lequel un spectateur devient pilote ; l'ancien pilote devient spectateur. Jamais automatique entre deux postes présents. _Éviter_ : « takeover » dans l'UI, « focus ».

Hors du pont, un `tmux attach` direct ou « Détacher » restent des clients tmux ordinaires, hors de ces rôles. Sécurité : un contrôle d'`Origin` garde **les deux** WebSockets (le terminal PTY *et* le flux d'événements du dashboard) contre le DNS-rebinding / CSWSH. L'allowlist par défaut est loopback (`localhost`/`127.0.0.1`) et **s'étend** par configuration pour un déploiement derrière reverse-proxy / domaine public — posture « Mono-user, local » inchangée (le daemon reste sans auth ni TLS ; le proxy les porte).
...
### Tours guidés — onboarding (#816, #909, ADR-0071)

**Tour** : parcours guidé nommé, en anglais dans l'UI, composé d'**étapes de tour** ; il pilote la vraie UI de l'instance et observe son vrai état, jamais une maquette ni un état simulé. Trois tours : *Overview*, le **tour préface** (visiter l'écran — canvas, panneaux, node, edge, Start/End, onglets, pastilles, archivage — sur un Run déjà terminé, sans harnais), *First run* (lancer un Run sur une pipeline à un node interactif, **puis lire sa vie jusqu'à l'output**) et *First pipeline* (implémenteur + testeur en boucle sur `verdict`). Le **full tour** les enchaîne dans cet ordre : entre deux tours, une **carte intermédiaire** dit où en est l'enchaînement et pose le choix explicitement — *Finish here* ou *Continue*. Un tour qui **s'arrête proprement** (cible absente, lancement refusé) ne perd pas l'enchaînement : sa carte propose le tour suivant, sans pour autant être marqué terminé. _Éviter_ : « tutoriel » pour désigner un parcours (c'est le nom du mode), « walkthrough », « démo ».

**Étape de tour** : une cible dans l'UI, une instruction (un impératif + deux phrases max), une condition d'avancement observée sur l'état réel (jamais un délai), et un drapeau *skippable* décidé étape par étape. Une saisie libre est validée par un bouton Next et propose un bloc à copier-coller. Une étape que l'app satisfait elle-même (le lancement ouvre l'onglet du Run) demande un Next elle aussi, et dit ce qui vient de se passer : une étape jamais affichée n'enseigne rien. Une cible absente au-delà de quelques secondes arrête le tour proprement, jamais de saut silencieux — **sauf une étape d'attente**, qui n'a aucune limite de temps (un agent met le temps qu'il met) et le dit à l'écran. Le tour n'agit jamais à la place de l'utilisateur ; il ne lui prend pas non plus ses touches : Échap appartient au terminal du node tant qu'il a le focus.

**Projecteur** : l'overlay qui grise tout l'écran sauf la cible de l'étape et bloque les clics hors cible ; quitter (le `✕ quit` du popover) et Échap restent toujours accessibles. Dans un menu ouvert, il se recentre sur l'option attendue. Une **zone souple** — pointillés, grisé plus clair — remplace la cible quand l'étape désigne un espace où circuler plutôt qu'un contrôle à viser : le menu d'une option, le panneau du node pendant une attente, ou l'artefact qu'on vient d'ouvrir — une même étape passe de la cible à la zone quand ce qu'elle désigne devient une page à lire. _Éviter_ : « spotlight » (nom UI, pas glossaire), « highlight », « coach mark ».

**Scène dégagée** : un tour démarre en congédiant les **surfaces flottantes** — les modales à fond plein écran qui bloquent l'app tant qu'elles sont là (la modale d'artefact). Le Projecteur peint au-dessus d'elles et désigne l'app *en dessous* : une modale oubliée resterait invisible et continuerait d'avaler les clics de la première étape. Une surface flottante se déclare congédiable sans rien savoir des tours (`lib/overlays.ts`) ; les Settings, eux, se ferment d'eux-mêmes puisque le clic vient de là. _Éviter_ : faire fermer une modale nommément par le tour.

**Cible atteignable** : une cible éclairée que le lecteur ne peut pas atteindre ne vaut pas mieux qu'une cible absente — le Projecteur lui possède la molette et le glissé, il ne reste que quitter. D'où deux règles. Une carte que l'app *crée* naît dans la **zone visible du canvas** : l'évitement de superposition raisonne en unités de canvas, la visibilité en pixels, et le cadrage automatique ne grossit jamais au-delà de 1:1 (`lib/nodePlacement.ts`). Une carte déjà là mais hors cadre se *montre* : le canvas se déclare capable d'amener un node au centre (`lib/canvasReveal.ts`, même patron que `lib/overlays.ts`), une fois par visée, jamais si elle est déjà à l'écran — le `scrollIntoView` du canvas, puisque faire défiler la page ne déplace pas une carte dans un viewport transformé. _Éviter_ : une étape non skippable dont la cible peut naître hors écran.

**Refus dit à voix haute** : une étape qui attend un geste que l'app vient de refuser attend pour toujours. Toute surface où un tour fait cliquer affiche donc le refus du daemon plutôt que de l'avaler (la modale New Pipeline sur un nom déjà pris), et l'étape correspondante s'arrête sur sa carte de refus en citant cette phrase. _Éviter_ : un `catch` silencieux sur un chemin qu'un tour emprunte.

**Préparation / rangement** : ce qu'un tour installe derrière sa carte d'intro avant la première étape (dépôt, pipeline, et pour *Overview* un vrai Run de nodes `script` et un Trigger d'exemple), idempotent ; et ce qu'il retire à **toute** sortie — fin, quitter, arrêt propre. Le rangement ne retire que ce qui a été posé pour être montré (le Trigger d'exemple, inoffensif par construction) ; ce que l'utilisateur garderait (pipeline, Run) reste à lui. La préparation n'est pas une étape : pendant les étapes, le tour n'agit jamais à la place de l'utilisateur (ADR-0071 §4). _Éviter_ : « setup »/« teardown » (noms de code, pas glossaire), « données de démo ».
...
- **Drop-in** *(terme)* : le fichier `pdo.service.d/override.conf` où l'opérateur pose durablement les variables env-only (`PDO_ALLOWED_WS_ORIGINS`) — il survit à chaque réécriture de l'unité, y compris celle de l'Update. Documenté en une phrase dans la table des commandes de `docs/reference/cli.md`, pas davantage.
