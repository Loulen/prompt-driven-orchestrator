...
**Runs terminés seulement** *(terme, #808 — completed only)* : bascule de la coquille Stats, à côté de la période, qui restreint la **cohorte** aux Runs arrivés à `completed`. Commune aux trois sections (Overview, Cost, Performance) ; Overview affiche alors zéro erreur, rien n'est masqué. Défaut : désactivée (cohorte = tous les Runs démarrés dans la période, quel que soit leur statut final, où un Node réussi dans un Run `failed` compte). _Éviter_ : « runs réussis » (un Run `completed` peut contenir des nœuds échoués puis relancés), « filtre de statut ».

**Durée active** *(terme, #808 — active duration)* : durée d'une exécution de Node dont on a **soustrait l'attente déclarée** : les intervalles entre un `NodeAwaitingUser` de cause `declared` ou `completion_not_released` et le premier `NodeResumed`, `NodeCompletionReleased` ou événement terminal du même `(node, iter)`. La **durée** (murale, `NodeStarted → NodeCompleted`) reste la mesure par défaut ; Performance offre une bascule **« exclude user wait »** entre les deux, les deux distributions étant toujours calculées. La ligne Infrastructure soustrait la somme des attentes des Nodes du Run. Ne sont **pas** soustraits : la cause `children_pending` (un orchestrateur sur `pdo run wait` travaille), et l'attente **dérivée** d'un parent dont un enfant attend (rien n'est écrit dans l'event log, ADR-0069 §4 : non mesurable, assumé). Un `NodeAwaitingUser` sans cause n'existe plus (ADR-0069 : jamais au spawn). _Éviter_ : « temps de travail », « durée nette », « temps d'attente » comme métrique à part (il n'y en a pas).
...
**Niveau de zoom d'un box-plot** *(terme, #808)* : réglage à trois positions, commun à tous les box-plots de Performance : **Full** (moustaches min–max, défaut), **Fenced** (moustaches aux **bornes de Tukey** : plus petite et plus grande observation dans `[Q1 − 1,5 IQR ; Q3 + 1,5 IQR]`, calculées par le daemon, les outliers sortent du cadre), **Box** (boîte Q1–Q3 et médiane seules). Aucun point individuel : le daemon ne renvoie jamais les observations. **Axe partagé** *(terme)* : par défaut l'axe X d'une métrique est commun à toutes les lignes visibles d'un tableau et se cale sur le max du niveau de zoom choisi ; la bascule **« independent scales »** donne à chaque ligne son propre max. Zoom, axe, bascules et filtres de Stats sont des préférences **par navigateur**, comme le thème. _Éviter_ : « outliers masqués » (ils sont hors cadre, pas retirés de la distribution), « échelle log ».
