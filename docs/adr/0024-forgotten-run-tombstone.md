...
3. **410 Gone aux frontières HTTP** : `run_command`, `node_done` et — depuis #465 (ADR-0042) — `PATCH /runs/{id}/repos` pré-vérifient le tombstone en tête de handler, avant tout side-effect (merge de sub-worktree en particulier). Toute autre surface répond 404 sur un run oublié (le log est purgé), jamais 410.
4. **Projection durcie** : `project()` retourne `None` si le log ne contient aucun `RunStarted` (`started_at` jamais posé). Tout call site production de `project()` est None-tolerant ; aucun run légitime n'existe sans `RunStarted` (appendé en premier à la création, avant worktree et scheduling).
