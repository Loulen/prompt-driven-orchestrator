...
- **La précédence du sandbox perd le tier `stored`.** Elle devient `profil (par Run, gelé) → env →
  ...
  défauts de profil** (`sandbox_profile::DEFAULT_PROFILE_IMAGE`, à côté de `DEFAULT_FULL_ENTRIES`).
  Un défaut n'a donc pas besoin d'être réglable *ici* pour exister ; c'est ce que le raisonnement de
  #467 avait confondu.
...
- **Une valeur stockée qui devient inopérante se dit une fois.** Les colonnes ne sont pas droppées
  (SQLite `DROP COLUMN` sur une install vivante n'achète rien) : elles deviennent **inertes**, et le
  boot émet **un** `warn!` qui les nomme et renvoie vers le profil à éditer. Une valeur que
  l'utilisateur a posée ne doit jamais cesser de compter en silence (même principe que #470). Ce
  chemin se couvre par un **test** qui seede la valeur, jamais par une observation manuelle : sur
  l'instance de référence il n'y en a aucune.
