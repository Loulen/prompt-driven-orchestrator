...
export default function LoopInspector() {
  ...
  if (!tab || !node || node.type !== "loop") return null;

  function handleMaxIterChange(value: string) {
    const trimmed = value.trim();
    if (trimmed.startsWith("$")) {
      updateNode(node!.id, { max_iter: trimmed });
    } else {
      const num = parseInt(trimmed, 10);
      if (!isNaN(num) && num > 0) {
        updateNode(node!.id, { max_iter: num });
      }
    }
  }
      ...
      <div className="flex flex-col gap-3 p-3" style={{ fontSize: "11.5px" }}>
        ...
        <Field label="max_iter">
          <div className="flex items-center gap-2">
            <input
              key={`${node.id}-maxiter`}
              data-testid="max-iter-input"
              defaultValue={node.max_iter ?? 5}
              onBlur={(e) => handleMaxIterChange(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  handleMaxIterChange((e.target as HTMLInputElement).value);
                  (e.target as HTMLInputElement).blur();
                }
              }}
        ...
        <p className="text-fg-4" style={{ fontSize: "10px", lineHeight: 1.5 }}>
          ...
          <code className="text-fg-3">done</code> (outputs). Side placement is
