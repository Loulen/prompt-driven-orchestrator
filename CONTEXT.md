...
- **Output porté** *(terme)* : le ou les ports source d'une edge. À la création, l'edge porte le **premier output déclaré** du node ; le choix se révise dans le panneau de détail de l'edge. _Éviter_ : « port de l'edge » (ambigu avec l'input cible), « edge par port ».
- **`when` et multi-port** : une clause `when` **désigne le port** dont elle lit la frontmatter ; le verdict s'applique à l'edge entière, donc à tous ses ports (ADR-0073).
